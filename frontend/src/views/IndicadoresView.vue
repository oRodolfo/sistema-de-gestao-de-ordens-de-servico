  ...
  <div class="p-8">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div class="space-y-3">
          <p v-if="!tecnicos.length" class="text-sm text-gray-400 text-center py-4">Nenhuma ordem atribuída a técnicos.</p>
...
// Estrutura inicial reativa totalmente segura
...
const cards = computed(() => [
  { label: 'Total sob Gestão', valor: dadosBrutos.value?.totalOrdens || 0, cor: 'text-blue-700', descricao: 'ordens atribuídas' },
  { label: 'Aguard. Aprovação', valor: dadosBrutos.value?.abertas || 0, cor: 'text-orange-500', descricao: 'requerem ação' },
...
// Mapeamento das Pendências de Ação
const pendentes = computed(() => {
  ...
  return [
    { label: 'Aguardando aprovação', valor: p.aguardando_aprovacao || 0, cor: 'text-orange-500' },
    { label: 'Aguardando material', valor: p.aguardando_material || 0, cor: 'text-yellow-600' },
    { label: 'Aguardando terceiro', valor: p.aguardando_terceiro || 0, cor: 'text-purple-600' },
    { label: 'Sem técnico atribuído', valor: p.sem_tecnico || 0, cor: 'text-red-500' },
  ]
})

// Mapeamento do Gráfico de Colunas Semanal
...
// Requisição Axios integrada à nova rota do Django
